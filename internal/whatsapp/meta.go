...
func (m *MetaProvider) Start(cfg *config.Config, dbDSN string, db *sql.DB, brain *nlp.Brain) error {
	...
	// Iniciamos asíncronamente para poder bloquear en main con <-c si es necesario o manejar errores
	go func() {
...
func (m *MetaProvider) processWebhookMessage(msg map[string]interface{}, valueMap map[string]interface{}) {
	if msgType, _ := msg["type"].(string); msgType != "text" {
		return // Solo soportamos texto por ahora, como indica WhatsApp Mau antes para notas de voz pero centrado en texto aquí
	}

	from, _ := msg["from"].(string)
	
	// Intentamos sacar el PushName de los contacts si Meta lo envía
	pushName := from
	...
	HandleIncomingMessage(msgText, from, pushName, m.db, m.brain, func(targetID, text string) error {
		return m.SendMessage(targetID, text)
	})
}

func (m *MetaProvider) SendMessage(target string, text string) error {
	// Limpiar JIDs/targets
